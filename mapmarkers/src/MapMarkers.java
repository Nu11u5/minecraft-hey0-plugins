...
// VEHICLE_POSITIONCHANGE not implemented for boats, using VEHICLE_UPDATE instead.   
/* 
    public void onVehiclePositionChange(BaseVehicle vehicle, int x, int y, int z) {
      try {
        //log.info(LOG_PREFIX + "VEHICLE_POSITIONCHANGE: entered for MapMarkers Plugin");
        Player passenger = vehicle.getPassenger();
        if (!vehicle.isEmpty() && passenger != null) {
          log.info(LOG_PREFIX + "VEHICLE_POSITIONCHANGE: " + passenger.getName() + " - " + x + ", " + y + ", " +z);
          setMarker(passenger.getName(), x, y, z, 4);
          if (available.tryAcquire()) {
            // Update file
            writeMarkers();
            // Set timer to release in 3 secs
    ...
    public void onVehicleEnter(BaseVehicle vehicle, HumanEntity player) {
      try {
        ...
        if (!vehicle.isEmpty() && passenger != null) {
          ...
          setMarker(passenger.getName(), passenger.getX(), passenger.getY(), passenger.getZ(), 4);
          // Update file
          writeMarkers();
          }
        else {
...
*/
    ...
    //Alternative to VEHICLE_ENTERED and VEHICLE_POSITIONCHANGE to work with boats.
    public void onVehicleUpdate(BaseVehicle vehicle) {
      try {
        Player passenger = vehicle.getPassenger();
        if (!vehicle.isEmpty() && passenger != null) {
          setMarker(passenger.getName(), vehicle.getX(), vehicle.getY(), vehicle.getZ(), 4);
          if (available.tryAcquire()) {
            //log.info(LOG_PREFIX + "VEHICLE_UPDATE: " + passenger.getName() + " - " + vehicle.getX() + ", " + vehicle.getY() + ", " +vehicle.getZ());
            // Update file
            writeMarkers();
            // Set timer to release in 3 secs
            Timer timer = new Timer();
